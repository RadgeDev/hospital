...
function mostrarDatos(valorBuscar, pagina, cantidad, valorcombo) {
    ...
    $.ajax({
        url: "http://localhost/hospital/control_historial_pedido/mostrar",
        type: "POST",
        ...
        success: function(response) {
            ...
            $.each(response.obtener, function(key, item) {
                filas += "<tr class='active' ><td >" + item.folio + "</td><td>" + item.depto + "</td><td>" + item.tipo_pedido + "</td><td>" + item.fecha + "</td><td>" + item.hora + "</td><td>" + item.nombre + "</td><td> <button href='" + item.folio + "'  id='editando'  onclick='editandos(this);' class='btn btn-success'>Ver</button></td></tr>";
            });
