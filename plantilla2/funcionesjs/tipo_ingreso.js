...
function main() {
    ...
    $("body").on("click", ".paginacion li a", function(e) {
        ...
        valoroption = $("#cantidad").val();
        mostrarDatos(valorBuscar, valorhref, valoroption, "cod_ingreso");
    });

    $("#cantidad").change(function() {
        valoroption = $(this).val();
        valorBuscar = $("input[name=busqueda]").val();
        mostrarDatos(valorBuscar, 1, valoroption, "cod_ingreso");
    });
}
...
function mostrarDatos(valorBuscar, pagina, cantidad, valorcombo) {
    ...
    $.ajax({
        url: "http://localhost/hospital/control_tipo_ingreso/mostrar",
        type: "POST",
        ...
        success: function(response) {
            ...
            filas = "";
            $.each(response.mostrar, function(key, item) {
                filas += "<tr class='active' ><td >" + item.cod_ingreso + "</td><td>" + item.nombre + "</td><td> <button href='" + item.cod_ingreso + "'  id='editando'  onclick='editandos(this);' class='btn btn-warning' data-toggle='modal' data-target='#myModalEditar'>E</button> <button href='" + item.cod_ingreso + "'  id='eliminando'  onclick='eliminar(this);' class='btn btn-danger' >X</button></td></tr>";
            });
...
function validarCodigo() {
    ...
    $.ajax({
        url: "http://localhost/hospital/control_tipo_ingreso/validar",
        type: "POST",
        data: { id: micodigo },
...
function editandos(obj) {
    ...
    $.ajax({
        url: "http://localhost/hospital/control_tipo_ingreso/editando",
        type: "POST",
        data: { id: codseleccionado },
        dataType: "json",
        success: function(respuesta) {
            $.each(respuesta.obtener, function(key, item) {
                $("#seleccod").val(item.cod_ingreso);
...
function eliminando(borrarcod) {
    $.ajax({
        ...
        url: "http://localhost/hospital/control_tipo_ingreso/eliminar",
        type: "POST",
...
function exportarexcel() {
    ...
    var url = "http://localhost/hospital/control_tipo_ingreso/excel"
    $.post(url, { sendData: JSON.stringify(obj) }, function(data) {
        ...
        $a.attr("download", "Reporte Tipo Ingreso " + fechatotal + ' ' + time + ".xls");
        $a[0].click();
        $a.remove();
    }, "json");
}