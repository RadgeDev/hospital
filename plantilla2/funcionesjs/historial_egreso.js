...
function main() {
    ...
    mostrarDatos("", 1, 10, "nombre_salida");
    $("#msg-error").hide();
...
function mostrarDatos(valorBuscar, pagina, cantidad, valorcombo) {
    ...
    $.ajax({
        url: "http://localhost/hospital/control_historial_egreso/mostrar",
        type: "POST",
        ...
        success: function(response) {
            ...
            $.each(response.obtener, function(key, item) {
                filas += "<tr class='active' ><td >" + item.cod_salida + "</td><td>" + item.nombre_salida + "</td><td>" + item.nombre_depto + "</td><td>" + item.fecha + "</td><td>" + item.nombre + "</td><td> <button href='" + item.cod_salida + "'  id='editando'  onclick='editandos(this);' class='btn btn-success'>Ver</button></td></tr>";
            });
