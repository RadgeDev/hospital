...
function guardarsalida() {
    ...
    switch (opcion) {
        ...
        case "4": //Salida directa
            ...
            break;
        default:
            swal("Algo fallo!", "Eliga tipo de salida.", "error");
    }
...
function cambiacontenidotabla() {
    var rowCount = $('#tbproductos tr').length;
    if (rowCount > 1) {
        $("#guardarsalida").prop("disabled", false);
        $("#imprimirsalida").prop("disabled", false);
        $("#limpiarsalida").prop("disabled", false);
        $("#Comentarios").prop("readonly", false);
        validartabla();
    } else {
        $("#guardarsalida").prop("disabled", true);
        $("#imprimirsalida").prop("disabled", true);
        $("#limpiarsalida").prop("disabled", true);
        $("#Comentarios").prop("readonly", true);
    }
...
function validartabla() {
    ...
    $("#tbproductos td:nth-child(4)").each(function() {
        var val = $(this).text().replace(" ", "").replace(",-", "");
        if (val == "N/N") {
            $("#guardarsalida").prop("disabled", true);
            $("#imprimirsalida").prop("disabled", true);
