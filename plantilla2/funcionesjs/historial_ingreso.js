...
function main() {
    ...
    mostrarDatos("", 1, 10, "tipo_compra_nombre");
    $("#msg-error").hide();
...
function mostrarDatos(valorBuscar, pagina, cantidad, valorcombo) {
    ...
    $.ajax({
        url: "http://localhost/hospital/control_historial_ingreso/mostrar",
        type: "POST",
        ...
        success: function(response) {
            ...
            $.each(response.obtener, function(key, item) {
                filas += "<tr class='active' ><td >" + item.cod_compra + "</td><td>" + item.tipo_documento + "</td><td>" + item.numero_documento + "</td><td>" + item.tipo_compra_nombre + "</td><td>" + item.nombre_proveedor +
                    "</td><td>" + item.fecha + "</td><td >" + item.nombre_usuario + "</td><td> <button href='" + item.cod_compra + "'  id='editando'  onclick='editandos(this);' class='btn btn-success'>Ver</button></td></tr>";
            });
