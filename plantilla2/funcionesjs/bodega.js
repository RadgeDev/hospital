...
function exportarexcel() {
    ...
    $("#tbclientes tbody tr").each(function(index) {
        var micodigo, minombre, micorrelativo;
        $(this).children("td").each(function(index2) {
                switch (index2) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        micorrelativo = $(this).text();
                        break;
                }

            })
            //  var obj = JSON.parse('[datostabla]');
        obj['datos'].push({ "codigo": micodigo, "nombre": minombre, "correlativo": micorrelativo });

        //var miJSON = JSON.encode(obj);
        //alert(campo1 + ' - ' + campo2 + ' - ' + campo3);
    })
    var url = "http://localhost/hospital/control_bodega/excel"
    $.post(url, { sendData: JSON.stringify(obj) }, function(data) {
        ...
        $a.attr("download", "Reporte Bodegas " + fechatotal + ' ' + time + ".xls");
        $a[0].click();
        $a.remove();
    }, "json");
}