...
class Consumo_model extends CI_Model {
  ...
  $this->db->select('Year(fecha) AS ano,MONTHNAME(CONCAT(fecha)) AS mes ,cod_producto ,bincard.nombre,cod_bodega,bodegas.nombre as nombrebodega, Sum(salida) as totales');
 ...
 $this->db->order_by('cod_producto', 'desc');
