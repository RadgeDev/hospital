...
class Control_tipo_ingreso extends CI_Controller {
 ...
 switch ($misesion) {
   case "Administrador":
         $tiponav= 'bodega/nav'; 
         $tipobody='bodega/vista_tipo_ingreso/view_tipo_ingreso';
         break;
   case "Bodeguero":
...
$this->phpexcel->getProperties()
            ->setTitle('Excel')
			->setDescription('Tipo Ingreso');
		     $data = json_decode($this->input->post('sendData'));
			$sheet=$this->phpexcel->getActiveSheet();
			$sheet->setTitle('Tipo Ingreso');
			$style = array(
                         'alignment' => array(
			...
			//generrar filas
			    ...
			    $sheet->setCellValue('A1','CODIGO TIPO INGRESO');
...
$i=3;
	...
	$nombre="Tipo Ingreso ".date(" Y-m-d H:i:s");
...
$this->phpexcel->getProperties()
			...
			$datos= $this->tipo_ingreso_model->get_tipo_ingreso();
			$sheet=$this->phpexcel->getActiveSheet();
			$sheet->setTitle('REPORTE Tipo Ingreso');
			$style = array(
                         'alignment' => array(
                          'horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_LEFT,
                           )
...
$i=3;
 foreach ($datos as $dato){
                $sheet->setCellValue('A'.$i, $dato->cod_ingreso);
	...
	//generar renderizacion
	header("Content-Type: application/vnd.ms-excel");
	$nombre="Reporte Tipo Ingreso ".date(" Y-m-d H:i:s");
	header("Content-Disposition: attachment; filename=\"$nombre.xls\"");
	header("Cache-Control: max-age=0");
	$writer=PHPExcel_IOFactory::createWriter($this->phpexcel,"Excel5");
	$writer->save("php://output");
