...
class Control_historial_ingreso extends CI_Controller {
	public function __construct(){
		parent::__construct();
		$this->load->model("Historial_ingreso_model");
	}
 ...
 switch ($misesion) {
   ...
   default:
	...
	{	
		...
		$data = array(
			"obtener" => $this->Historial_ingreso_model->buscar($buscar,$inicio,$cantidad,$combobuscar),
			"totalregistros" => count($this->Historial_ingreso_model->buscar($buscar)),
			"cantidad" =>$cantidad
...
public function mostrarfecha()
	{	
		...
		$inicio = ($numeropagina -1)*$cantidad;
		$data = array(
			"obtener" => $this->Historial_ingreso_model->buscarfecha($fechaini,$inicio,$cantidad,$fechafin),
			"totalregistros" => count($this->Historial_ingreso_model->buscarfecha($fechaini,$fechafin)),
			"cantidad" =>$cantidad
	...
	{	
		...
		$inicio = ($numeropagina -1)*$cantidad;
		$data = array(
			"obtener" => $this->Historial_ingreso_model->buscar2($buscar,$inicio,$cantidad,$combobuscar),
			"totalregistros" => count($this->Historial_ingreso_model->buscar2($buscar)),
			"cantidad" =>$cantidad
...
{    
           ...
           $this->phpexcel->getProperties()
			...
			$datos= $this->Historial_ingreso_model->get_fechasvencimiento($fechaini,$fechafin);
...
{    
            ...
            ->setTitle('Productos Venimiento')
			->setDescription('Vencimientos');
			$datos= $this->Historial_ingreso_model->porvencercritico();
			$sheet=$this->phpexcel->getActiveSheet();
			$sheet->setTitle('Productos por vencer');
			$style = array(
                         'alignment' => array(
...
{    
            ...
            ->setTitle('Productos Venimiento')
			->setDescription('Vencimientos');
			$datos= $this->Historial_ingreso_model->porvencerminimo();
			$sheet=$this->phpexcel->getActiveSheet();
			$sheet->setTitle('Productos por vencer');
			$style = array(
                         'alignment' => array(
...
{    
            ...
            ->setTitle('Productos Venimiento')
			->setDescription('Vencimientos');
			$datos= $this->Historial_ingreso_model->porvencermaximo();
			$sheet=$this->phpexcel->getActiveSheet();
			$sheet->setTitle('Productos por vencer');
			$style = array(
                         'alignment' => array(
